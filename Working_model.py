...
def apply_professional_dark_theme(root):
    # Color palette
    COLORS = {
        "bg": "#1E1E1E",
        "bg_secondary": "#252526",
    ...
    style.configure('.', background=COLORS["bg"], foreground=COLORS["text"], font=("Segoe UI", 12))
    style.configure('TLabel', background=COLORS["bg"], foreground=COLORS["text"])
    style.configure('Heading.TLabel', background=COLORS["bg"], foreground=COLORS["heading"],
    ...
    style.map('TButton',
              background=[('active', COLORS["button_active"]), ('pressed', COLORS["button_hover"]),
                          ('!active', COLORS["bg_secondary"])],
              foreground=[('active', COLORS["heading"]), ('!active', COLORS["text"])]
              )
    ...
    # Custom widget styling for rounded corners and underline
    def style_modern_button(widget):
        widget.configure(
            bg=COLORS["bg_secondary"],
            fg=COLORS["text"],
            activebackground=COLORS["button_active"],
            activeforeground=COLORS["heading"],
            relief="flat",
            bd=0,
        ...
        # Rounded corners via canvas overlay (optional, for advanced UI)

    def style_modern_label(widget, heading=False):
        widget.configure(
            bg=COLORS["bg"],
            fg=COLORS["heading"] if heading else COLORS["text"],
            font=("Segoe UI", 18, "bold") if heading else ("Segoe UI", 14)
        )

    def style_modern_entry(widget):
        widget.configure(
            bg=COLORS["bg_secondary"],
            fg=COLORS["text"],
            ...
            highlightcolor=COLORS["entry_underline"],
            font=("Segoe UI", 14),
            bd=0
        )

    # Hover effect for buttons
    def add_button_hover(widget):
        def on_enter(e):
            widget['bg'] = COLORS["button_active"]

        def on_leave(e):
            widget['bg'] = COLORS["bg_secondary"]

        widget.bind("<Enter>", on_enter)
        widget.bind("<Leave>", on_leave)

    # Attach styling functions to root for later use
    root._style_modern_button = style_modern_button
    root._style_modern_label = style_modern_label
    root._style_modern_entry = style_modern_entry
    root._add_button_hover = add_button_hover
    root._theme_colors = COLORS


# Patch Application __init__ to apply the new theme and styles
if "Application" in globals():
    old_init = Application.__init__


    def new_init(self, *args, **kwargs):
        old_init(self, *args, **kwargs)
        apply_professional_dark_theme(self.root)
        c = self.root._theme_colors

        # Style all labels
        for label in [self.panel, self.panel2, self.T, self.panel3, self.T1, self.panel5, self.T3, self.T4]:
            heading = (label == self.T)
            self.root._style_modern_label(label, heading=heading)
        # Style all buttons
        for button in [self.b1, self.b2, self.b3, self.b4, self.speak, self.clear]:
            self.root._style_modern_button(button)
            self.root._add_button_hover(button)
        # Style suggestion label
        self.T4.configure(fg=c["accent"])
        # Set background for main window and panels
        self.root.configure(bg=c["bg"])
        self.panel.configure(bg=c["bg"])
        self.panel2.configure(bg=c["bg"])
        self.panel3.configure(bg=c["bg"])
        self.panel5.configure(bg=c["bg"])


    Application.__init__ = new_init


def style_modern_button(button):
    button.configure(
        bg="#44475a",
        fg="#f8f8f2",
...
def apply_dark_modern_theme(root):
    root.configure(bg="#23272e")
    for widget in root.winfo_children():
        if isinstance(widget, tk.Button):
            style_modern_button(widget)
        elif isinstance(widget, tk.Label):
            style_modern_label(widget)


# Patch Application to apply theme after widgets are created
old_init = None
if "Application" in globals():
    old_init = Application.__init__
    ...
    def new_init(self, *args, **kwargs):
        old_init(self, *args, **kwargs)
        apply_dark_modern_theme(self.root)
...
class Application:
    ...
    def __init__(self):
        ...
        for i in ascii_uppercase:
            self.ct[i] = 0
        print("Loading the opencv Model....")
        self.root = tk.Tk()
        self.root.title("SPEAK HANDS: Made by Tanush")
        ...
        self.T.config(text="SPEAK HANDS: An AI-driven system for real-time American Sign Language recognition", font=("Courier", 40, "bold"))
        ...
        # Apply professional dark theme to all widgets
        apply_professional_dark_theme(self.root)
        c = self.root._theme_colors
    ...
    def video_loop(self):
        try:
                    ...
                    if image.all:
                        ...
                        if handz[0]:
                            ...
                            cv2.line(white, (self.pts[5][0] + os, self.pts[5][1] + os1),
                                     (self.pts[9][0] + os, self.pts[9][1] + os1), (0, 0, 0),
                                     3)
                            cv2.line(white, (self.pts[9][0] + os, self.pts[9][1] + os1),
                                     (self.pts[13][0] + os, self.pts[13][1] + os1), (0, 0, 0),
                                     3)
                            ...
                            cv2.line(white, (self.pts[0][0] + os, self.pts[0][1] + os1),
                                     (self.pts[5][0] + os, self.pts[5][1] + os1), (0,0, 0),
                                     3)
                            cv2.line(white, (self.pts[0][0] + os, self.pts[0][1] + os1),
                                     (self.pts[17][0] + os, self.pts[17][1] + os1), (0, 0, 0),
                                     3)

                            for i in range(21):
                                cv2.circle(white, (self.pts[i][0] + os, self.pts[i][1] + os1), 2, (0, 0, 0), 1)
            ...
            if hands:
                ...
                # white = cv2.imread("C:\\Users\\dev\\PycharmProjects\\pythonProject\\white.jpg")
                # img_final=img_final1=img_final2=0
